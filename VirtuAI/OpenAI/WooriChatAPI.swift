...
struct QuickReplyButton: Codable {
    let label: String
...
struct WooriMessageData: Codable {
    let content: String
    let sender: String
    let createdAt: String
    var quickReplyButtons: [QuickReplyButton]?
}
...
class WooriChatAPI: ObservableObject {
    ...
    @Published public private(set) var messages: [WooriMessageData] = []
    ...
    private func generateMessages(from text: String, systemText: String) -> [Message] {
        var messages = [systemMessage(content: systemText)] + historyList + [Message(role: "ROLE_MEMBER", content: text)]
    ...
    public func addWooriMessage(_ message: WooriMessageData) {
        DispatchQueue.main.async {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            ForEach(message.quickReplyButtons ?? [], id: \.label) { button in
                Button(action: {
                    print("Action: \(button.actionValue.text ?? "No text available")")
                }) {
                    VStack {
                        Text(button.label)
                            .fontWeight(.bold)
