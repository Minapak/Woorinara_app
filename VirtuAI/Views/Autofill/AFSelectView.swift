...
//  VirtuAI
//
//  Created by 박은민 on 11/6/24.
//

import SwiftUI
import PDFKit

struct AFSelectView: View {
    ...
    var body: some View {
        VStack {
            ...
            ZStack {
                ...
                AFSelectViewRepresentable(document: document, checkBoxStates: $checkBoxStates, checkBoxCoordinates: checkBoxCoordinates, checkBoxSize: checkBoxSize) { location in
                    // 특정 영역 내 클릭 시 모달 표시
        ...
        .sheet(isPresented: $showModal) {
            AFSelectModalView(selectedIndex: $selectedIndex) { index in
                if let index = index {
...
struct AFSelectViewRepresentable: UIViewRepresentable {
