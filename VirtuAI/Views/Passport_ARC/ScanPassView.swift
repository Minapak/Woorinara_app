...
struct ScanPassView: View {
    ...
    private func savePassportData() {
        ...
        do {
            ...
            print("Failed to encode passport data: \(error.localizedDescription)")
        }
    }

    private func loadPassportData() -> [String: String]? {
        guard let savedData = UserDefaults.standard.data(forKey: "SavedPassportData") else {
            return nil
        }
        
        do {
            return try JSONDecoder().decode([String: String].self, from: savedData)
        } catch {
            print("Failed to decode passport data: \(error.localizedDescription)")
            return nil
        }
    }
}
